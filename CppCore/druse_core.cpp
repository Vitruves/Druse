...
DruseMoleculeResult* druse_parse_mmcif(const char *content) {
    if (!content || !content[0]) return make_error("Empty mmCIF content");

    try {
        std::istringstream stream(content);
        std::string line;

        // Parse _atom_site category column names
        ...
        struct AtomSiteMeta {
            ...
            std::string atomName;
            int residueSeq = 0;
            bool isHetAtom = false;
        };
        std::vector<AtomSiteMeta> metadata;

        // Column index mapping
        int col_group = -1, col_symbol = -1, col_atom_id = -1;
        int col_comp_id = -1, col_asym_id = -1, col_seq_id = -1;
        int col_x = -1, col_y = -1, col_z = -1;
        int col_occupancy = -1, col_bfactor = -1;
        ...
        while (std::getline(stream, line)) {
            ...
            if (line.empty()) {
                if (inAtomSite && !readingColumns && !rows.empty())
            ...
            if (line.find("_atom_site.") == 0) {
                inAtomSite = true;
                readingColumns = true;
                std::string colName = line.substr(11);
                // Trim whitespace from column name
                size_t pos = colName.find_first_of(" \t");
                if (pos != std::string::npos) colName = colName.substr(0, pos);

                int idx = (int)columnNames.size();
                columnNames.push_back(colName);

                if (colName == "group_PDB") col_group = idx;
                else if (colName == "type_symbol") col_symbol = idx;
                else if (colName == "label_atom_id" || colName == "auth_atom_id") {
                    if (col_atom_id < 0) col_atom_id = idx;
                }
                else if (colName == "label_comp_id" || colName == "auth_comp_id") {
                    if (col_comp_id < 0) col_comp_id = idx;
                }
                else if (colName == "label_asym_id" || colName == "auth_asym_id") {
                    if (col_asym_id < 0) col_asym_id = idx;
                }
                else if (colName == "label_seq_id" || colName == "auth_seq_id") {
                    if (col_seq_id < 0) col_seq_id = idx;
                }
                else if (colName == "Cartn_x") col_x = idx;
                else if (colName == "Cartn_y") col_y = idx;
                else if (colName == "Cartn_z") col_z = idx;
                else if (colName == "occupancy") col_occupancy = idx;
                else if (colName == "B_iso_or_equiv") col_bfactor = idx;

                continue;
            }

            if (inAtomSite && readingColumns && line[0] != '_') {
                readingColumns = false;
            }

            if (line[0] == '#' || line.find("loop_") == 0) {
                if (inAtomSite && !rows.empty()) break;
                if (line.find("loop_") == 0) {
                    if (inAtomSite && rows.empty()) continue;
                }
                continue;
            }

            if (inAtomSite && !readingColumns && line[0] != '_' && line[0] != '#') {
                // Parse data row - tokenize by whitespace (handle quoted strings)
                ...
                if (!tokens.empty()) {
                    AtomSiteMeta meta;
                    if (col_comp_id >= 0 && col_comp_id < (int)tokens.size()) meta.residueName = tokens[col_comp_id];
                    if (col_asym_id >= 0 && col_asym_id < (int)tokens.size()) meta.chainID = tokens[col_asym_id];
                    if (col_atom_id >= 0 && col_atom_id < (int)tokens.size()) meta.atomName = tokens[col_atom_id];
                    if (col_seq_id >= 0 && col_seq_id < (int)tokens.size()) {
                        const std::string &seq = tokens[col_seq_id];
                        if (!seq.empty() && seq != "." && seq != "?") {
                            try { meta.residueSeq = std::stoi(seq); } catch (...) {}
                        }
                    }
                    if (col_group >= 0 && col_group < (int)tokens.size()) {
                        meta.isHetAtom = tokens[col_group] == "HETATM";
                    }
        ...
        // Suppress unused-but-set for columns used in future expansions
        (void)col_occupancy; (void)col_bfactor;

        if (rows.empty() || col_x < 0 || col_y < 0 || col_z < 0) {
            return make_error("No _atom_site data found in mmCIF");
        }

        int natoms = (int)rows.size();
        auto *res = new DruseMoleculeResult();
        memset(res, 0, sizeof(DruseMoleculeResult));
        res->success = true;
        strncpy(res->name, "mmcif_structure", sizeof(res->name) - 1);
        res->atomCount = natoms;
        res->atoms = new DruseAtom[natoms];

        // Element symbol to atomic number mapping
        auto element_to_num = [](const std::string &sym) -> int {
        ...
        for (int i = 0; i < natoms; i++) {
            ...
            // Coordinates
            if (col_x < (int)row.size()) da.x = std::stof(row[col_x]);
            if (col_y < (int)row.size()) da.y = std::stof(row[col_y]);
            if (col_z < (int)row.size()) da.z = std::stof(row[col_z]);

            // Element symbol
            std::string sym = "C";
            if (col_symbol >= 0 && col_symbol < (int)row.size()) sym = row[col_symbol];
            strncpy(da.symbol, sym.c_str(), sizeof(da.symbol) - 1);
            da.atomicNum = element_to_num(sym);

            // Atom name
            if (col_atom_id >= 0 && col_atom_id < (int)row.size()) {
                strncpy(da.name, row[col_atom_id].c_str(), sizeof(da.name) - 1);
            }

            const AtomSiteMeta &meta = metadata[i];
            const std::string residueName = meta.residueName.empty() ? "UNK" : meta.residueName;
            const std::string chainID = meta.chainID.empty() ? "A" : meta.chainID;
            const std::string atomName = meta.atomName.empty() ? sym : meta.atomName;
            strncpy(da.name, atomName.c_str(), sizeof(da.name) - 1);
            strncpy(da.residueName, residueName.c_str(), sizeof(da.residueName) - 1);
            strncpy(da.chainID, chainID.c_str(), sizeof(da.chainID) - 1);
            da.residueSeq = meta.residueSeq == 0 ? (i + 1) : meta.residueSeq;
            da.isHetAtom = meta.isHetAtom;
        }

        auto covalent_radius = [](int atomicNum) -> float {
            switch (atomicNum) {
                case 1:  return 0.31f;
                case 6:  return 0.76f;
                case 7:  return 0.71f;
        ...
        auto is_same_residue = [&](int i, int j) -> bool {
            const DruseAtom &a = res->atoms[i];
            const DruseAtom &b = res->atoms[j];
            return a.residueSeq == b.residueSeq &&
                   strcmp(a.chainID, b.chainID) == 0 &&
                   strcmp(a.residueName, b.residueName) == 0;
        };

        auto is_peptide_link = [&](int i, int j) -> bool {
            const DruseAtom &a = res->atoms[i];
            const DruseAtom &b = res->atoms[j];
            if (a.isHetAtom || b.isHetAtom) return false;
            if (strcmp(a.chainID, b.chainID) != 0) return false;
            return (strcmp(a.name, "C") == 0 && strcmp(b.name, "N") == 0 && b.residueSeq == a.residueSeq + 1) ||
                   (strcmp(b.name, "C") == 0 && strcmp(a.name, "N") == 0 && a.residueSeq == b.residueSeq + 1);
        };

        auto is_disulfide = [&](int i, int j) -> bool {
            const DruseAtom &a = res->atoms[i];
            const DruseAtom &b = res->atoms[j];
            return strcmp(a.residueName, "CYS") == 0 &&
                   strcmp(b.residueName, "CYS") == 0 &&
                   strcmp(a.name, "SG") == 0 &&
                   strcmp(b.name, "SG") == 0;
        };

        std::vector<DruseBond> bondVec;

        for (int i = 0; i < natoms; i++) {
            for (int j = i + 1; j < natoms; j++) {
                float dx = res->atoms[i].x - res->atoms[j].x;
                float dy = res->atoms[i].y - res->atoms[j].y;
                float dz = res->atoms[i].z - res->atoms[j].z;
                float d2 = dx*dx + dy*dy + dz*dz;
                if (d2 <= 0.16f) continue;

                const float dist = std::sqrt(d2);
                bool shouldBond = false;

                if (is_same_residue(i, j)) {
                    const float cutoff = covalent_radius(res->atoms[i].atomicNum) +
                                         covalent_radius(res->atoms[j].atomicNum) + 0.45f;
                    shouldBond = dist <= cutoff;
                } else if (is_peptide_link(i, j)) {
                    shouldBond = dist <= 1.7f;
                } else if (is_disulfide(i, j)) {
                    shouldBond = dist <= 2.3f;
                }
        ...
        res->bondCount = (int)bondVec.size();
        if (!bondVec.empty()) {
            res->bonds = new DruseBond[bondVec.size()];
            memcpy(res->bonds, bondVec.data(), bondVec.size() * sizeof(DruseBond));
        }
        ...
        return make_error("Unknown error parsing mmCIF");
    }
}
