...
struct ImportColumnMapping: Identifiable {
    ...
    var target: ImportTargetField = .none
}
...
struct ImportMappingSheet: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Actions
            HStack {
                let assignedCount = preview?.columns.filter { $0.target != .none }.count ?? 0
                Text("\(assignedCount) column(s) assigned")
                    .font(.system(size: 10))
                    .foregroundStyle(.secondary)
    ...
    private func bindingForColumn(_ idx: Int) -> Binding<ImportTargetField> {
        Binding(
            get: { preview?.columns[idx].target ?? .none },
            set: { newValue in
