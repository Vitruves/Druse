...
struct StatusStripView: View {
    ...
    private var progressFraction: Float {
        if viewModel.isBatchDocking {
            let bp = viewModel.batchProgress
            return bp.total > 0 ? Float(bp.current) / Float(bp.total) : 0
        }
        if viewModel.screeningProgress > 0 && viewModel.screeningProgress < 1 {
            return viewModel.screeningProgress
        }
        return 0
    }

    private var isActive: Bool {
        viewModel.isDocking || viewModel.isBatchDocking || viewModel.isMinimizing
            || viewModel.isGeneratingSurface || viewModel.isLoading || viewModel.isSearching
    }
    ...
    private var statusInfo: (icon: String, message: String, color: Color) {
        ...
        if viewModel.isGeneratingSurface {
            return ("drop.halffull", "Generating surface...", .blue)
        }
        if viewModel.isLoading {
            return ("arrow.down.circle", viewModel.loadingMessage, .blue)
        }
        ...
        let msg = viewModel.statusMessage
    ...
    private var contextualHint: String? {
        ...
        if !viewModel.proteinPrepared && viewModel.protein != nil {
            return "Tip: Run Preparation to clean the structure"
        }
        if viewModel.ligand == nil && viewModel.protein != nil {
            return "Add a ligand from the Ligands tab to dock"
        }
        if viewModel.ligand != nil && viewModel.selectedPocket == nil && viewModel.dockingResults.isEmpty {
            return "Detect a binding site in the Docking tab"
        }
        if viewModel.selectedPocket != nil && viewModel.dockingResults.isEmpty {
            return "Ready to dock — configure and run"
        }
        if !viewModel.dockingResults.isEmpty {
            let best = viewModel.dockingResults.first?.energy ?? 0
            return String(format: "Best pose: %.2f kcal/mol — check Results", best)
        }
        return nil
    }
                ...
                Rectangle()
                    ...
                    .gesture(
                        DragGesture(minimumDistance: 1)
                            .onChanged { value in
                                consoleHeight = max(60, min(500, consoleHeight - value.translation.height))
                            }
                    )
                    .overlay(
